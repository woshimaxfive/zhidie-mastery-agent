...
页面下方展示完整学习路径：诊断理解、渐进引导、迁移验证和更新证据。当前所处的环节会被标记出来，已经走过的环节和尚未到达的环节在视觉上有所区分。系统会根据已有会话自动切换“开始诊断”“继续学习”或“开始迁移验证”。
...
窄屏下改为单列，Agent 当前决定排在任务卡之前，不需要滚到页面底部查看。
...
答错时，展开区域的标题会变成“你的理解展开”，节点也换成琥珀色描边。这里呈现的是学习者本次答案的推演过程，不是正确执行结果。

如果暂时无法修正，可以点击“我需要一点提示”。提示最多分为三级，只增加当前需要的信息，不直接替代学习者完成作答。停止值、起始值和步长等不同错因会进入不同的提示路径。已经解锁的提示会一并保留，可以回看每一层分别补充了什么信息。

![步长错因对应的专属提示](images/user-guide/personalized-hint.png)
...
回到学习首页后，系统会提供“开始迁移验证”入口。新会话不会携带上一次任务的提示记录，并会换用不同的目标序列，避免把记忆答案当作独立迁移能力。
...
掌握后，首页会同时提供“查看掌握证据”和“再练一次”。再次练习会建立新的诊断会话并继续积累证据，不会删除已有记录。

![已掌握后的再次练习入口](images/user-guide/repractice.png)
